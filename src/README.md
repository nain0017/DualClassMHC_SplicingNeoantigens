...
Open `../notebooks/pipeline.ipynb` in Google Colab Pro with a T4 GPU high-RAM runtime. The notebook is self-contained and calls out to the software pinned in `../environment.yaml` and `../requirements.txt`. All 240 cells execute in order and produce the outputs archived under Supplementary Tables T1 to T10.
